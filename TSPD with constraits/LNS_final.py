...
# --------------------------
# Hàm best_insertion: chèn lại các node bị loại theo phương pháp best insertion
# (Tính chi phí chèn và chọn vị trí có chi phí nhỏ nhất)
# --------------------------
...
# --------------------------
# Hàm random_removal: xóa ngẫu nhiên các node (ngoại trừ depot)
# --------------------------
def random_removal(city_tour, neighborhood_size):
    """
    Loại bỏ ngẫu nhiên 'neighborhood_size' thành phố từ city_tour (ngoại trừ depot ở đầu và cuối).
    Trả về: (danh sách các node bị loại, danh sách tour sau khi loại bỏ)
    """
    removed = random.sample(city_tour[1:-1], neighborhood_size)
    remaining = [t for t in city_tour if t not in removed]
    return removed, remaining

# --------------------------
# Hàm random_removal_insertion: áp dụng LNS theo phương pháp xóa và chèn
# sử dụng best_insertion thay vì random_insertion
# --------------------------
def random_removal_insertion(particle, coordinates, distance_matrix, neighborhood_size=4):
    ...
    Áp dụng LNS bằng cách:
      ...
      2. Chèn lại các node đã loại bỏ bằng phương pháp best insertion.
    ...
    # Tạo lại particle với tất cả gán xe tải ban đầu
...
# --------------------------
# Hàm tìm kiếm giải pháp tổng thể (LNS + 2-opt)
# --------------------------
def greedy_search_emission(coordinates, iterations=100, neighborhood_size=4, verbose=True):
    ...
    # Generation 1: Random Removal & Reinsertion (LNS)
...
def main():
    ...
    while i < 20:
        ...
        records, best_particle, best_fitness, truck_distance = greedy_search_emission(
            coordinates, iterations=100, neighborhood_size=4, verbose=True)
        ...
        export_csv(records, output_file="Result/output_200.csv")
        i += 1
